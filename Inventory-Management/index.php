...
if(isset($_POST['submit'])) {
    ...
    $result = mysqli_query($conn,"SELECT product_code FROM inventory WHERE product_code = '$product_code'");
    if(mysqli_num_rows($result) == 0) {
        ...
        if($_FILES['image']['name'] == "") {
            ...
            $query = "INSERT INTO `inventory`(`id`, `product_code`, `product_name`, `description`, `categoryid`, `unit_cost`, `quantity`, `supplierid`, `image` ,`status` ,`date`) 
                    VALUES (NULL,'$product_code','$product_name ','$description','$id_category','$unit_cost','$quantity','$id_supplier', '$newImageName','enable','$itemdate');";
        ...
        echo
            "<script> alert('Product Code is already existed!'); </script>"
        ;
    }
            ...
            <div class="container">
                <div class="table" style="">
                    <div class="table_header">
                        ...
                        <a href="index-disabled.php">
                            <button class="add_new" id="show-login">View Disabled</button>
                        </a>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                include "db_conn.php";
                                    ...
                                    $sql = "SELECT inv.*,cat.category_name,sup.supplier_name FROM inventory inv, category cat,suppliers sup WHERE `status` = 'enable' && inv.categoryid = cat.id && inv.supplierid = sup.id";
                                    $result = mysqli_query($conn,$sql);
                                    function getColor($n){
                                        // Is number less than 11
                                        if($n<30) return "red";
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)){
                                        ...
                                        <tr style="color:<?=$color?>">
                                            ...
                                            <td>     
                                                <a href="edit-inventory.php?id=<?php echo $row['id'] ?>"><i class='bx bxs-edit' style="font-size: 25px"></i></a>
                                                <a href="disable-inventory.php?id=<?php echo $row['id'] ?>"><i class='bx bx-low-vision' style="font-size: 25px"></i></a>
                                                <a href="delete-inventory.php?id=<?php echo $row['id'] ?>" style="display:<?=$validation?>"  ><i class='bx bx-trash' style="font-size: 25px"></i></a>  
                                            </td>
                                        </tr>
                                        <?php
                                    }
